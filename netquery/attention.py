...
class IntersectConcatAttention(nn.Module):
    ...
    def forward(self, query_embed, key_embeds, mode):
        ...
        # 为了解决使用高版本pytorch的时候，此处出现AttributeError: 'torch.return_types.min' object has no attribute 'unsqueeze'的错误
        # 此处将query_embed_expand = query_embed.unsqueeze(0).expand_as(key_embeds)
        # 修改为query_embed_expand = query_embed[0].unsqueeze(0).expand_as(key_embeds)
        # 这里是一个tuple（value,indice）
