    ...
    for i in range(2, 4):
        for kind in ["val", "test"]:
            if kind == "val":
                ...
                to_keep = 10000
            test_queries = load_queries_by_type("/dfs/scratch0/nqe-bio/{:s}_queries_{:d}-split.pkl".format(kind, i),
                                                keep_graph=True)
            print("Loaded", i, kind)
            for query_type in test_queries:
                test_queries[query_type] = [q for q in test_queries[query_type] if
                                            len(q.get_edges().intersection(deleted_edges)) > 0]
            ...
            pickle.dump(test_queries, open("/dfs/scratch0/nqe-bio/{:s}_queries_{:d}-clean.pkl".format(kind, i), "wb"),
...
def make_train_test_edge_data(data_dir, neg_sample_size):
    ...
    pickle.dump([q.serialize() for q in valid_queries], open(data_dir + "/val_edges.pkl", "wb"),
                protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in test_queries], open(data_dir + "/test_edges.pkl", "wb"),
                protocol=pickle.HIGHEST_PROTOCOL)

    print("Dumping train 1-chain queries")
    train_queries = [Query(("1-chain", e), None, None, keep_graph=True) for e in train_triples]
    pickle.dump([q.serialize() for q in train_queries], open(data_dir + "/train_edges.pkl", "wb"),
                protocol=pickle.HIGHEST_PROTOCOL)
...
def _discard_negatives(file_name, small_prop=0.9):
    ...
    queries = [q if random.random() > small_prop else (
        q[0], [random.choice(list(q[1]))], None if q[2] is None else [random.choice(list(q[2]))]) for q in queries]
...
def make_train_multiedge_query_data(data_dir, num_workers, samples_per_worker, mp_result_dir=None, id2geo=None):
    ...
    pickle.dump([q.serialize() for q in queries_2],
                open(data_dir + "/train_queries_2{}.pkl".format(file_postfix), "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump([q.serialize() for q in queries_3],
                open(data_dir + "/train_queries_3{}.pkl".format(file_postfix), "wb"), protocol=pickle.HIGHEST_PROTOCOL)


def make_train_inter_query_data(data_dir, num_workers, samples_per_worker, max_inter_size=5, mp_result_dir=None,
                                id2geo=None):
    '''
    ...
    for arity in queries_dict:
        pickle.dump([q.serialize() for q in queries_dict[arity]],
                    open(data_dir + "/train_inter_queries_{:d}{:s}.pkl".format(arity, file_postfix), "wb"),
                    protocol=pickle.HIGHEST_PROTOCOL)

def make_train_test_val_geo(data_dir,time):
    train_edges = pickle_load(data_dir+'/train_edges.pkl')
    train_edges = pickle.load(open(data_dir+'/train_edges.pkl', "rb"), encoding="utf-8")
    test_edges = pickle.load(open(data_dir+'/test_edges.pkl', "rb"))
    val_edges = pickle.load(open(data_dir+'/val_edges.pkl', "rb"))

    id2time = pickle.load(open(data_dir+time, "rb"))
    timeid_list = list(id2time.keys())
    ...
    for edges in train_edges:
        if edges[0][1][2] in timeid_list:
            train_edges_geo.append(edges)

    for edges in test_edges:
        if edges[0][1][2] in timeid_list:
            test_edges_geo.append(edges)

    for edges in val_edges:
        if edges[0][1][2] in timeid_list:
            val_edges_geo.append(edges)

    pickle.dump(train_edges_geo,open(data_dir + "/train_edges-geo.pkl", "wb"))
    print('train_edges-geo extract done!')
    pickle.dump(test_edges_geo,open(data_dir + "/test_edges-geo.pkl", "wb"))
    print('test_edges-geo extract done!')
    pickle.dump(val_edges_geo,open(data_dir + "/val_edges-geo.pkl", "wb"))
    print('val_edges-geo extract done!')
...
if __name__ == "__main__":
    ...
    # 1.1 make train_edges-geo.pkl/test_edges-geo.pkl/valid_edges-geo.pkl
    time = '/id2location.pkl'
    # make_train_test_val_geo(data_dir,time)
    ...
    id2geo = pickle_load(data_dir + "/id2location.pkl")
