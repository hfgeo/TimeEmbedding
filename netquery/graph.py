...
class Graph():
    ...
    def get_negative_samples(self, query, id2geo = None):
        ...
        if query[0] == "3-chain" or query[0] == "2-chain":
            ...
            # 一样 xubing 2023-10-3 00:09:55
            # pos_nodes = set([n for neigh in inter_neighs for n in self.adj_lists[rel][neigh]])
            new_pos_nodes = set()
            for neigh in inter_neighs:
                neighbors = self.adj_lists[rel].get(neigh)
                if neighbors is not None:
                    new_pos_nodes.update(neighbors)

            pos_nodes = new_pos_nodes

            # 同样修改键不存在 --- xubing -- 2023-10-3 00:02:33
            # union_pos_nodes = set([n for neigh in union_neighs for n in self.adj_lists[rel][neigh]])
            new_union_pos_nodes = set()
            for neigh in union_neighs:
                neighbors = self.adj_lists[rel].get(neigh)
                if neighbors is not None:
                    new_union_pos_nodes.update(neighbors)

            union_pos_nodes = new_union_pos_nodes
            # end  -- 2023-10-3 00:09:06
    ...
    def get_metapath_neighs(self, node, rels):
        ...
        for rel in rels:
            ...
            # 修改以下键找不到时的情况 -- xubing 2023-10-2 23:50:42
            # current_set = set([neigh for n in current_set for neigh in self.adj_lists[rel].get(n) if neigh is not None])
            # current_set.discard(None)

            new_set = set()
            for n in current_set:
                neigh_list = self.adj_lists[rel].get(n)
                if neigh_list is not None:
                    new_set.update(neigh_list)

            current_set = new_set

            # new_set = set()
            # for n in current_set:
            #     res = self.adj_lists[rel].get(n,'1')
            #     if res != '1':
            #         new_set.add(res)
            # current_set = new_set
