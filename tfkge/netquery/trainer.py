...
class Trainer():
    ...
    def __init__(self, args, graph, feature_modules, node_maps, out_dims, 
        ...
        '''
        # 2024-1-6 14:04:18 此处使用tmp判断接收的参数数量
        # xubing
        # 注意    此处的 id2geo,只有在融合位置和时间时才表示 location,其他时候表示time
        # 沿用了之前代码的逻辑，未做更改

        tmp = self.load_id2geo()
        if len(tmp)==3:
            # 融合 location + time
            self.id2geo, self.id2extent, self.id2geo_time = tmp
        elif len(tmp)==2:
            # 融合 time
            self.id2geo, self.id2extent = tmp

            # 由于在现有函数中加入了 id2geo_time 参数， 这里需要手动初始化，但实际在后续未使用，仅为初始化
            self.id2geo_time = None


        # self.id2geo, self.id2extent = self.load_id2geo()

        #  end ---------- 2024-1-6 14:04:52
        id2geo = self.id2geo
        ...
        self.log_file = args.log_dir + self.formatted_time + args.spa_enc_type  +".log"
        self.model_file = args.model_dir + self.formatted_time + args.spa_enc_type  + ".pth"

        if self.args.load_model:
            # self.model_file = 'G:\\xubing\\14time\se-kge-master\graphqa\model_dir\yagotime\\2023-11-05-13-44-01Time2VecEncode.pth'
            # self.model_file = 'G:\\xubing\\14time\se-kge-master\graphqa\model_dir\yagotime\\Time2VecEncode.pth'
            # self.model_file = 'G:\\xubing\\14time\se-kge-master\graphqa\model_dir\yagotime\\..-0-no---64-2048-0.005000-0.010000-bilinear-min_simple-F-concat_norm_postm-leakyrelu-sigmoid-1-F-T-7.pth'
            self.model_file = 'G:\\xubing\\14time\se-kge-master\graphqa\model_dir\only_time\\2024-01-06-15-28-42no.pth'

        self.set_evel_file_name()


        self.logger = setup_logging(self.log_file, console = False, filemode='a')
        ...
        if args.spa_enc_type != "no":
            self.spa_enc, self.time_enc = get_spa_encoder(args = args,
                    geo_info = args.geo_info,
                    spa_enc_type = args.spa_enc_type,
                    id2geo = id2geo,
                    spa_embed_dim = args.spa_embed_dim,
        ...
        if args.enc_agg_type == "concat":
            ...
            if args.geo_info == "time_loc":
                model_out_dims = {mode: out_dims[mode] + args.spa_embed_dim*2 for mode in out_dims}
    ...
    def load_id2geo(self):
        ...
        if self.args.spa_enc_type == 'no':
            ...
            if self.args.geo_info == "geo":
                ...
                id2extent = None
            elif self.args.geo_info == "time":
                id2geo = pickle_load(self.args.data_dir + "/id2time.pkl")
                # 此处添加当时间直接编码时，归一化读入的时间  --xubing 2023-10-12 15:30:06
                if self.args.spa_enc_type=="time_direct_encode":
                    # 初始化数据最大的年份
                    max_year = 1
                    for key, value in id2geo.items():
                        max_year = abs(max(max_year, abs(value[0])))
                    for key, value in id2geo.items():
                        id2geo[key] = [value[0] / max_year, value[1] / 12 / max_year, value[2] / 12 / 30 / max_year,
                                       value[3] / 12 / 30 / 24 / max_year, value[4] / 12 / 30 / 24 / 60 / max_year,
                                       value[5] / 12 / 30 / 24 / 60 / 60 / max_year]
                id2extent = None

            #  时间+位置编码
            #  在之前的代码中，是使用id2geo表示时间，这里为了表示方面，用id2geo表示location
            #  id2geo_time表示该方法的时间
            elif self.args.geo_info == "time_loc":
                id2geo = pickle_load(self.args.data_dir + "/id2location.pkl")
                id2geo_time = pickle_load(self.args.data_dir + "/id2time.pkl")
                id2extent = (-180, 180, -90, 90)
                return id2geo, id2extent,id2geo_time
    ...
    def load_model_1(self,file):
        self.logger.info("Load model from {}".format(file))
        self.enc_dec.load_state_dict(torch.load(file))
    ...
    def set_evel_file_name(self):
        ...
        self.val_geo_prec_detail_log_file_add = self.args.log_dir + self.args_combine + "--fm_prec_geo_Valid______add.pkl"
        self.test_geo_prec_detail_log_file_add = self.args.log_dir + self.args_combine + "--fm_prec_geo_Test______add.pkl"

        self.val_prec_detail_log_file_add = self.args.log_dir + self.args_combine + "--fm_prec_Valid______add.pkl"
        self.test_prec_detail_log_file_add = self.args.log_dir + self.args_combine + "--fm_prec_Test______add.pkl"
    ...
    def eval_model(self, flag="TEST"):
        ...
        self.test_auc_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_auc_Test.pkl"
        self.test_geo_auc_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_auc_geo_Test.pkl"
        self.val_auc_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_auc_Valid.pkl"
        self.val_geo_auc_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_auc_geo_Valid.pkl"

        self.test_prec_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_Test.pkl"
        self.test_geo_prec_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_geo_Test.pkl"
        self.val_prec_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_Valid.pkl"
        self.val_geo_prec_detail_log_file = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_geo_Valid.pkl"

        self.val_geo_prec_detail_log_file_add = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_geo_Valid______add.pkl"
        self.test_geo_prec_detail_log_file_add = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_geo_Test______add.pkl"

        self.val_prec_detail_log_file_add = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_Valid______add.pkl"
        self.test_prec_detail_log_file_add = self.args.log_dir + self.formatted_time + self.args.spa_enc_type + "--fm_prec_Test______add.pkl"
        ...
        if flag == "TEST":
            ...
            geo_prec_detail_log_file_add = self.test_geo_prec_detail_log_file_add

            prec_detail_log_file_add = self.test_prec_detail_log_file_add
            ...
            geo_prec_detail_log_file_add = self.val_geo_prec_detail_log_file_add
            prec_detail_log_file_add = self.val_prec_detail_log_file_add

            tag = "Valid"
        
        if self.args.eval_general:
            if self.args.eval_log:
                v, aprs, qtype2fm_auc, qtype2fm_q_prec,qtype2fm_q_prec_add = run_eval(self.enc_dec, queries, 0, self.logger, eval_detail_log = True)
                ...
                pickle_dump(qtype2fm_q_prec_add, prec_detail_log_file_add)
            else:
                v, aprs = run_eval(self.enc_dec, queries, 0, self.logger)
            self.logger.info("{} macro-averaged AUC: {:f}, APR: {:f}".format(tag, np.mean(list(v.values())), np.mean(list(aprs.values()))))
        if self.args.geo_train:
            if self.args.eval_geo_log:
                v_geo, aprs_geo, qtype2fm_auc_geo, qtype2fm_q_prec_geo,qtype2fm_q_prec_geo_add = run_eval(self.enc_dec, queries_geo, 0, self.logger, geo_train = True, eval_detail_log = True)
                ...
                pickle_dump(qtype2fm_q_prec_geo_add, geo_prec_detail_log_file_add)
            else:
                v_geo, aprs_geo = run_eval(self.enc_dec, queries_geo, 0, self.logger, geo_train = True)
            self.logger.info("GEO {} macro-averaged AUC: {:f}, APR: {:f}".format(tag, np.mean(list(v_geo.values())), np.mean(list(aprs_geo.values()))))
