...
class HexagonGridCellSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, frequency_num = 16, 
        max_radius = 10000, dropout = 0.5, f_act = "sigmoid", device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
...
class TheoryDiagGridCellSpatialRelationEncoder(nn.Module):
    ...
    def __init__(self, spa_embed_dim, coord_dim = 2, frequency_num = 16, 
        ...
        freq_init = "geometric", use_layn=False, use_post_mat = False, device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
        ...
        self.post_mat = nn.Parameter(torch.FloatTensor(self.frequency_num, 6, self.spa_embed_dim//self.frequency_num).to(self.device))
        init.xavier_uniform_(self.post_mat)
        self.register_parameter("spa_postmat", self.post_mat)
        self.dropout = nn.Dropout(p=dropout)
...
class NaiveSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, extent, coord_dim = 2, ffn = None, device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
...
class DirectSpatialRelationEncoder(nn.Module):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function
    ...
    def __init__(self, spa_embed_dim, extent, coord_dim=2, ffn=None, device="cpu"):
        ...
        Args:
            ...
            extent: (x_min, x_max, y_min, y_max)
        """
        super(DirectSpatialRelationEncoder, self).__init__()
        self.spa_embed_dim = spa_embed_dim
        self.coord_dim = coord_dim
        self.extent = extent
...
class PolarCoordSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, ffn = None, device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
    ...
    def forward(self, coords):
        ...
        # spr_embeds: shape (batch_size, num_context_pt, coord_dim)
        spr_embeds = torch.FloatTensor(polars).to(self.device)
...
class PolarGridCoordSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, frequency_num = 16, 
        max_radius = 10000, min_radius = 10, freq_init = "geometric", 
        ffn = None, device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
...
class RBFSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, id2geo, spa_embed_dim, coord_dim = 2, anchor_sample_method = "fromid2geo",
                num_rbf_anchor_pts = 100, 
                rbf_kernal_size = 10e2, 
                rbf_kernal_size_ratio = 0.0, 
        ...
        self.anchor_sample_method = anchor_sample_method
        self.id2geo = id2geo
        self.spa_embed_dim = spa_embed_dim
        self.coord_dim = coord_dim
    ...
    def cal_rbf_anchor_coord_mat(self):
        if self.anchor_sample_method == "fromid2geo":
            ...
            x_list = np.random.uniform(self.extent[0], self.extent[1], self.num_rbf_anchor_pts)
            # x_list[0] = 0.0
            y_list = np.random.uniform(self.extent[2], self.extent[3], self.num_rbf_anchor_pts)
            # y_list[0] = 0.0
            # self.rbf_coords: (num_rbf_anchor_pts, 2) 
            ...
            raise Exception("Unrecognized anchor_sample_method!!!!")
...
class GridLookupSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, interval = 300, extent = None, 
        ffn = None, device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
    ...
    def make_grid_embedding(self, interval, extent):
        ...
        self.embedding = torch.nn.Embedding(self.num_col * self.num_row, self.spa_embed_dim).to(self.device)
    ...
    def make_input_embeds(self, coords):
        ...
        spr_embeds = self.embedding(torch.autograd.Variable(index_mat).to(self.device))
...
class PolarGridLookupSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, coord_dim = 2, max_radius = 10000, 
        frequency_num = 16, 
        ffn = None, device = "cpu"):
        """
        Args:
    ...
    def make_polar_grid_embedding(self):
        ...
        self.embedding = torch.nn.Embedding((self.frequency_num+self.eps) **2 , self.spa_embed_dim).to(self.device)
        self.embedding.weight.data.normal_(0, 1./self.spa_embed_dim)
...
class AodhaSpatialRelationEncoder(nn.Module):
    ...
    """
    def __init__(self, spa_embed_dim, extent, coord_dim = 2, 
        num_hidden_layers = 4, hidden_dim = 256, use_post_mat = True, 
        f_act = "relu", device = "cpu"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
