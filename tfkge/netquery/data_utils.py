...
def parallel_sample_for(pid, num_samples, graph, data_dir, is_test, test_edges, mp_result_dir = None, id2geo = None):
    '''
    Args:
        id2geo: node id => [longitude, latitude]
                if not None, we sample geographic query with target node as geographic entity
...
def parallel_sample(graph, num_workers, samples_per_worker, data_dir, test=False, start_ind=None, mp_result_dir = None, id2geo = None):
    ...
    for i in range(num_workers):
        parallel_sample_for(i, samples_per_worker, graph, data_dir, test, val_edges+test_edges, mp_result_dir=mp_result_dir,id2geo=id2geo)
...
def parallel_inter_query_sample_for(pid, num_samples, graph, data_dir, is_test, max_inter_size=5, mp_result_dir=None,
                                       id2geo=None):
    '''
    Args:
        id2geo: node id => [longitude, latitude]
...
def parallel_inter_query_sample(graph, num_workers, samples_per_worker, data_dir, max_inter_size = 5, test=False, start_ind=None, mp_result_dir = None, id2geo = None):
    ...
    for i in range(num_workers):
        parallel_inter_query_sample_for(i, samples_per_worker, graph, data_dir, test, max_inter_size, mp_result_dir, id2geo)
