...
from netquery.TimeEncoder import *
...
def _get_perc_index(scores, all_neg_queries,lengths):
    '''
    percentile rank score: Given a query, one positive target cos score p, x negative target, and their cos score [n1, n2, ..., nx],
    See the rank of p in [n1, n2, ..., nx]
    ...
    for i, length in enumerate(lengths):
        ...
        ls = neg_scores[cum_sum:cum_sum + length]
        sorted_indexes = sorted(range(len(ls)), key=lambda k: ls[k], reverse=True)
        neg_queries = all_neg_queries[cum_sum:cum_sum + length]
        sorted_neg_queries = [neg_queries[i] for i in sorted_indexes]
        index.append(sorted_neg_queries)
...
def eval_perc_queries(test_queries, enc_dec, batch_size=1000, hard_negatives=False, eval_detail_log = False):
    ...
    for formula in test_queries:
        ...
        while offset < len(formula_queries):
            ...
            all_neg_queries = [q.target_node for q in batch_queries] + negatives
            ...
            batch_perc_index = _get_perc_index(batch_scores,all_neg_queries, lengths)
            ...
            if eval_detail_log:
                ...
                for i, prec in enumerate(batch_perc_scores):
                    ...
                    if formula_key not in fm2query_prec_add:
                        fm2query_prec_add[formula_key]=[[query.anchor_nodes,query.target_node,query,prec,batch_perc_index[i]]]
                    else:
                        fm2query_prec_add[formula_key].append([query.anchor_nodes,query.target_node,query,prec,batch_perc_index[i]])

                    if query.query_graph is None:
                        continue
                    q_s = query.serialize()
                    fm2query_prec[formula_key].append([q_s, prec])

    if eval_detail_log:
        return np.mean(perc_scores), fm2query_prec,fm2query_prec_add
    else:
...
def get_pos_encoder(geo_info, 
    ...
    if geo_info in ["geo", "proj"]:
        ...
        pos_enc = ExtentPositionEncoder(spa_enc_type, id2geo, id2extent, spa_enc, graph, 
                                    spa_enc_embed_norm = spa_enc_embed_norm, device = device)
    elif geo_info in ["time"]:
        pos_enc = TimeEncoder(spa_enc_type, id2geo, spa_enc, graph,
                                    spa_enc_embed_norm = spa_enc_embed_norm, device = device)
    elif geo_info in ["time_loc"]:
        pos_enc = TimeLocationEncoder(spa_enc_type, id2geo,id2geo_time, spa_enc,time_enc, graph,
                                    spa_enc_embed_norm = spa_enc_embed_norm, device = device)
    else:
...
def get_metapath_decoder(graph, out_dims, decoder, feat_dims, spa_embed_dim, enc_agg_type):
    ...
    # 此处测试不同的解码器效果    *-*xubing 2023-10-31 22:11:41
...
def get_spa_encoder(args, geo_info, spa_enc_type, id2geo, spa_embed_dim, coord_dim = 2,
    ...
    #         ffn = ffn)
        ...
        spa_enc = None
    elif spa_enc_type == "time_forward_NN":
        ffn = get_ffn(args,
            input_dim= 6 ,
            f_act = f_act,
            context_str = "TimeForwardEncoderNN")
        spa_enc = TimeForwardEncoderNN(
            spa_embed_dim,
            coord_dim = coord_dim,
            ffn=ffn,
            device=device)
    elif spa_enc_type == "time_direct_encode":
        spa_enc = TimeDirectEncoder(
            spa_embed_dim,
            coord_dim=coord_dim,
            ffn=None,
            device=device)
    elif spa_enc_type == "Time2VecEncode":
        spa_enc = Time2VecEncoder(
            spa_embed_dim,
            coord_dim=coord_dim,
            ffn=None,
            device=device)
    #  融合时间和位置
    elif spa_enc_type == "time_loc_encoder":

        ffn = get_ffn(args,
            input_dim=2,
            f_act = f_act,
            context_str = "DirectSpatialRelationEncoder")
        spa_enc = DirectSpatialRelationEncoder(
            spa_embed_dim,
            extent = (-180, 180, -90, 90),
            coord_dim = coord_dim,
            ffn = ffn,
            device=device)
        time_enc = Time2VecEncoder(
            spa_embed_dim,
            coord_dim=coord_dim,
            ffn=None,
            device=device)

    elif spa_enc_type == "time_period_encoder":

        ffn = get_ffn(args,
                      input_dim=512,
                      f_act=f_act,
                      context_str="TheoryTimeRelationEncoder")

        single_ffn =  MultiLayerFeedForwardNN(
            input_dim=6,
            output_dim=args.spa_embed_dim,
            num_hidden_layers=args.num_hidden_layer,
            dropout_rate=args.dropout,
        ...
        spa_enc = TheoryTimeRelationEncoder(
            spa_embed_dim,
            min_time=100,
            max_time=2018,
            use_log=True,
    ...
    elif spa_enc_type == "time_period_new":
        ...
        single1_ffn =  MultiLayerFeedForwardNN(
            input_dim=24,
            output_dim=args.spa_embed_dim,
            num_hidden_layers=args.num_hidden_layer,
            dropout_rate=args.dropout,
        ...
        spa_enc = TheoryEncoder_NewStructure(
            spa_embed_dim,
            min_time=100,
            max_time=2018,
            use_log=True,
            ...
            device=device,
            is_time_period=True,
            single_ffn=(single_ffn,single1_ffn))

    elif spa_enc_type == "time_period_new_global":
        ...
        spa_enc = TheoryEncoder_NewStructure_global(
            spa_embed_dim,
            min_time=100,
            max_time=2018,
            use_log=True,
