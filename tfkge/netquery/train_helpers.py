import numpy as np
from netquery.utils import eval_auc_queries, eval_perc_queries, eval_auc_queries_spa_sem_lift, eval_perc_queries_spa_sem_lift
import torch
...
def check_conv(vals, window=2, tol=1e-6):
    ...
    if len(vals) < 2 * window:
        return False
    conv = np.mean(list(vals[-window:])) - np.mean(list(vals[-2*window:-window]))
    return conv < tol
...
def run_eval(model, queries, iteration, logger, by_type=False, geo_train = False, eval_detail_log = False):
    ...
    for query_type in queries["one_neg"]:
        ...
        if eval_detail_log:
            ...
            '''
            perc, fm2query_prec,fm2query_prec_add = perc_tuple
            qtype2fm_q_prec[query_type] = fm2query_prec
            qtype2fm_auc[query_type] = reauc_aucs
            qtype2fm_q_prec_add[query_type]=fm2query_prec_add
        else:
            # perc: average percentiel rank (APR) for current query type
            perc = perc_tuple
        vals[query_type] = auc
        ...
        if "inter" in query_type:
            ...
            if eval_detail_log:
                ...
                qtype2fm_q_prec_add[query_type + ":hard"]=fm2query_prec_add
            else:
                # perc: average percentiel rank (APR) for current query type
                perc = perc_tuple
    ...
    if eval_detail_log:
        return vals, aprs, qtype2fm_auc, qtype2fm_q_prec,qtype2fm_q_prec_add
    else:
        return vals, aprs
...
def run_train(model, optimizer, 
    ...
    for i in range(max_iter):
        ...
        # if edge_conv=False and (the model is edge c'c or len(losses) >= max_burn_in)
        if not edge_conv and (check_conv(vals, tol=tol) or len(losses) >= max_burn_in):
            ...
            conv_test_apr = np.mean(list(conv_test_apr.values()))

            if geo_train:
                logger.info("geo query...")
                conv_geo_test, conv_geo_test_apr = run_eval(model, test_queries_geo, i, logger, geo_train = True)
                conv_geo_test = np.mean(list(conv_geo_test.values()))
                conv_geo_test_apr = np.mean(list(conv_geo_test_apr.values()))
        ...
        if i >= val_every and i % val_every == 0:
            ...
            logger.info("Validate macro-averaged AUC: {:f}, APR: {:f}".format(np.mean(list(v.values())), np.mean(list(aprs.values()))))
            if geo_train:
                logger.info("geo query...")
                v_geo, aprs_geo = run_eval(model, val_queries_geo, i, logger, geo_train = True)
                logger.info("GEO Validate macro-averaged AUC: {:f}, APR: {:f}".format(np.mean(list(v_geo.values())), np.mean(list(aprs_geo.values()))))

            if edge_conv:
                if geo_train:
                    # vals.append(np.mean(v.values() + v_geo.values()))
                    vals.append(np.mean(list(v_geo.values())))
                else:
    ...
    if conv_test is not None:
        ...
        logger.info("APR Improvement from edge conv: {:f}".format((np.mean(list(aprs.values()))-conv_test_apr)/conv_test_apr))

    if geo_train:
        logger.info("geo query...")
        v_geo, aprs_geo = run_eval(model, test_queries_geo, i, logger, geo_train = True)
        logger.info("GEO Test macro-averaged AUC: {:f}, APR: {:f}".format(np.mean(list(v_geo.values())), np.mean(list(aprs_geo.values()))))
        if conv_geo_test is not None:
            logger.info("GEO AUC Improvement from edge conv: {:f}".format((np.mean(list(v_geo.values()))-conv_geo_test)/conv_geo_test))
            logger.info("GEO APR Improvement from edge conv: {:f}".format((np.mean(list(aprs_geo.values()))-conv_geo_test_apr)/conv_geo_test_apr))
...
def run_train_spa_sem_lift(model, optimizer, 
    ...
    for i in range(max_iter):
        ...
        if i >= val_every and i % val_every == 0:
            ...
            if geo_train:
                vals.append(np.mean(list(v.values()) + list(v_geo.values())))
